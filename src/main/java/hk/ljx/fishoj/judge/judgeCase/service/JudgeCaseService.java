...
public class JudgeCaseService {
     ...
     * @return 判题用例 VO 列表
     */
    public List<JudgeCaseVO> listCases(Long submitId) {
        ...
        List<JudgeCase> list = judgeCaseMapper.selectList(new LambdaQueryWrapper<JudgeCase>()
                .eq(JudgeCase::getSubmitId, submitId)
                .orderByAsc(JudgeCase::getId));
        return BeanUtil.copyToList(list, JudgeCaseVO.class);
    }
}